...
proc transpose data=rev_transposed out=rev_transposed (drop = _name_ _label_) prefix=rev_cntr;
    ...
    var &rev_cntr;
run;
*transpose the hcpcs to 1 row per bene/clm;
proc sort data=include_cohort1e nodupkey out=hcpcs_transposed; by &bene_id &clm_id &hcpcs_cd; run;
proc transpose data=hcpcs_transposed out=hcpcs_transposed (drop = _name_ _label_) prefix=hcpcs_cd;
    by &bene_id &clm_id ;
    var &hcpcs_cd;
run;
...
if pop_74150=1 and pop_74160=1 then keep;
if pop_74170=1 then keep;
