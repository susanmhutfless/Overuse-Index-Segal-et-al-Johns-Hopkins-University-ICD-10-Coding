...
%global bene_id 	clm_id  	clm_from_dt 	clm_thru_dt  			
		bene_dob 	bene_sex 	bene_race 		STATE_CODE COUNTY_CD ZIP_CD
...
%let vars_to_keep_mbsf    = 	&bene_id &ENROLLMT_YR &buyin &hmo &ZIP_CD &STATE_CODE &COUNTY_CD &bene_dob. &bene_sex. &bene_race.;
...
Data &CFI_person_out.; 
	set all;
	length STATE_COUNTY_CD $ 10;
	STATE_COUNTY_CD = trim(left(STATE_CODE))||trim(left(COUNTY_CD));
...
/*** Percent of participants with CFI > 0.2 by county ***/
...
proc sort data=&CFI_person_out.; by STATE_COUNTY_CD; run;
proc sort data=county_decile; by STATE_COUNTY_CD; run;
proc sort data=county_pct; by STATE_COUNTY_CD; run;

/*** Cell counts per county ***/
data county_n(keep=STATE_COUNTY_CD total_n frail_n nonfrail_n);
	set &CFI_person_out.;
	by STATE_COUNTY_CD;
	if first.STATE_COUNTY_CD then do; total_n=0; frail_n=0; nonfrail_n=0; end;
	total_n+1;
	if cfrail=0 then nonfrail_n+1;
		else if cfrail=1 then frail_n+1;
	if last.STATE_COUNTY_CD;
run;

data county;
	merge county_decile(drop=_type_ _freq_ where=(STATE_COUNTY_CD^="")) county_pct(drop=_type_ _freq_ where=(STATE_COUNTY_CD^="")) county_n;
	by STATE_COUNTY_CD;
...
proc datasets;
  delete include_cohort ip ip2 include_cohort;
run;
...
proc datasets;
  delete Bcarrier_base Inpatient_base Outpatient_base allclaims allclaimsnodup Bcarrier_line Inpatient_revenue Outpatient_revenue rev rev2 revnodup;
run;
...
proc print data=county_n noobs; var STATE_COUNTY_CD total_n frail_n nonfrail_n; where 0<total_n<11 or 0<frail_n<11 or 0<frail_n<11; run;
